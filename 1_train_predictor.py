...
parser.add_argument('--data', type=str, default='tecdata',
                    help='type of the dataset (ecg, gesture, power_demand, space_shuttle, respiration, nyc_taxi, tecdata')
parser.add_argument('--filename', type=str, default='tecdata.pkl',
...
def generate_output(args,epoch, model, gen_dataset, disp_uncertainty=True,startPoint=2500, endPoint=3500):
    if args.save_fig:
        ...
        for i in range(target.size(-1)):
            ...
            #              color='skyblue', marker='.', linestyle='--', markersize=1.5, linewidth=1)
            plt.plot(range(startPoint, endPoint), outSeq[startPoint:endPoint,i].numpy(), label='Recursive predictions for target'+str(i),
...
if not args.pretrained:
    ...
    try:
        for epoch in range(start_epoch, args.epochs+1):
            ...
            generate_output(args,epoch,model,gen_dataset,startPoint=2500)
